...
- **Recomendaciones basadas en reglas, no en IA**: un motor de reglas (`buildRecommendations`) detecta señales concretas — dinero acumulado sin destino en cuentas corrientes, tasa de ahorro por debajo de un umbral, fondo de emergencia incompleto, carga de deuda alta, cashflow negativo, cuentas concretas en descubierto, patrimonio por debajo del recomendado para la edad, varias deudas activas a la vez, ingresos o cuentas sin diversificar, fondo de emergencia completo sin ninguna inversión en marcha — y las prioriza por severidad. Determinista y explicable, no una caja negra.
- **El sobrante de una cuenta no tapa el descubierto de otra, ni en la cabecera de "Ingresos y Gastos" ni en las recomendaciones.** `totalAccountShortfall`/`totalAccountSurplus` operan sobre `balanceByAccount` cuenta a cuenta, no sobre el neto global de `totalMonthlyIncome`/`totalMonthlyExpenses`: si BBVA cierra en -1500€ e ING en +725€, lo que "falta" es 1500€, no el neto de -775€, porque el sobrante de ING es su propio ahorro, no dinero ya movido a BBVA. Cuando alguna cuenta está en descubierto, `buildRecommendations` lo señala como recomendación (moviendo manualmente el sobrante de otras cuentas si lo hay), nunca como una compensación automática.
...
- **Propiedades: valor de mercado suma al patrimonio, el alquiler no se cuenta dos veces.** `currentNetWorth` suma el valor estimado de las propiedades a los seguimientos de ahorro/inversión (menos la deuda pendiente, que ya incluye cualquier hipoteca dada de alta como deuda genérica — no hace falta vincular explícitamente propiedad↔hipoteca). El alquiler que puedan dar es un flujo, no un saldo: `incomes`/`expenses` tienen un `propertyId` (FK real a `properties.id`, `ON DELETE SET NULL`) y `rentalProfitByProperty` calcula el beneficio neto agrupando por esa relación, solo con fines informativos, sin sumarlo de nuevo al patrimonio — ya cuenta como ingreso normal en `totalMonthlyIncome`. Existe también un campo `property` de texto libre, independiente y sin relación, para una nota que no tiene por qué corresponder a una propiedad real.
- **`propertyId` es una FK real, a diferencia de cómo las cuentas se referencian por nombre.** Antes de esta relación, `property` (texto) se comparaba carácter a carácter contra `properties.name` para calcular `rentalProfitByProperty` — frágil ante un typo o una mayúscula distinta, y sin ninguna limpieza si la propiedad se borraba (la nota se quedaba huérfana para siempre). Al ser `propertyId` una FK de Postgres con `ON DELETE SET NULL`, borrar una propiedad limpia el vínculo automáticamente sin ningún código de cascada a mano — a diferencia del borrado de cuentas (referenciadas por nombre en 4 tablas distintas), que si necesita cascada manual porque ahí sí sería una migración de mayor calado convertir esas referencias en FKs reales.
...
- **Borrado de cuentas, en cascada.** Las cuentas bancarias son una entidad propia en la base de datos (no solo un texto suelto en cada ingreso/gasto), y sus ingresos/gastos/transferencias/seguimientos de ahorro la referencian solo por nombre (no hay FK real). Borrar una cuenta borra en la misma transacción todo lo que la referencia por nombre — la alternativa (bloquear el borrado hasta vaciar la cuenta a mano) se probó primero y generaba "cuentas fantasma": en cuanto algo se quedaba sin borrar (una transferencia, un seguimiento de ahorro), la cuenta parecía "no poder borrarse del todo" y su nombre seguía reapareciendo en la pantalla de gastos aunque ya no existiera como cuenta real.
...
## Perfil (singleton) y primera instalación sin pasos manuales

- **`profile` es una tabla de exactamente una fila**, no un CRUD más: `GET/PUT /profile` sin id en la URL (`profileRoutes.ts`), y el repositorio hace `update` sin `where` porque solo puede haber una fila. Sustituye a lo que antes era `src/data/finances.ts` (config estática ignorada por git: edad fija, `accountFlows`, objetivo del fondo de emergencia) — la edad ahora se calcula en el dominio (`calculateAge`) a partir de una fecha de nacimiento real editable, en vez de ser un número que había que mantener a mano.
- **Perfil no tiene sección propia en el menú, a propósito.** Se edita desde un modal que abre un botón en "Resumen" (`<Modal>` compartido, ver `src/components/Modal.tsx`, extraído del que ya usaba "Importar movimientos" para no duplicar el mismo cascarón de overlay/cabecera/cerrar dos veces): con 7 secciones ya en el menú, añadir una octava solo para algo que se edita una vez y casi nunca se vuelve a tocar no compensaba — un botón contextual en la pantalla donde ya se usa ese dato (la edad entra en el patrimonio recomendado de "Resumen") es más descubrible que un ítem de nav para una acción tan poco frecuente.
- **`accountFlows` era la causa real de las "cuentas fantasma".** Antes de esto, `balanceByAccount` añadía a la lista de cuentas mostradas cualquier nombre presente en `accountFlows` (config estática), sin comprobar si esa cuenta seguía existiendo de verdad. Si se borraba una cuenta que en algún momento apareció ahí, su nombre seguía resucitando en la pantalla de gastos — sin botón de "Eliminar cuenta" (porque ya no había ningún `Account` real con ese nombre), pero sin desaparecer tampoco. Al quitar `accountFlows` por completo, la única fuente de nombres de cuenta son las cuentas reales de la base de datos y los movimientos que las referencian — una cuenta borrada, con su cascada, no puede volver a aparecer.
- **`resetDatabase` (tests) no trunca `profile` a propósito** (ver comentario en `testPool.ts`): es una tabla singleton sembrada una vez por migración, no una colección de fixtures que tenga sentido vaciar entre tests.
- **Primera instalación sin copiar archivos a mano.** La migración `1783291400000_seed-demo-data` mete un perfil por defecto y datos de ejemplo (mismas cuentas/ingresos/gastos/deuda que antes traía `seed.example.ts`) dentro de un bloque `DO $$ ... $$` que comprueba "¿está `accounts` vacío?" antes de insertar nada — así que en una instalación ya existente con datos reales no toca nada (salvo sembrar `profile`, que es una tabla nueva y por tanto siempre vacía la primera vez que corre esta migración ahí también). El único paso que queda para arrancar de cero es `npm run migrate:up`; `seed.ts`/`npm run seed` pasan a ser una vía opcional para cargar datos reales por script en vez de desde la UI.

## Importación de extractos bancarios

Cada banco exporta sus movimientos en un formato distinto y sin categorías fiables; en vez de pedir que se traduzcan a mano, `domain/statementImport.ts` normaliza los formatos reales que maneja a la misma forma interna, agrupa transacciones del mismo comercio y las compara contra los gastos ya apuntados en la app — todo en el navegador, sin subir el archivo a ningún sitio.

- **El CSV/XLS es genérico por banco, el PDF no.** `findColumnMap` no busca "ING": busca cabeceras que *contengan* palabras como `IMPORTE`, `DESCRIP`/`CONCEPTO` o `FECHA`, así que cualquier extracto en `.xls`/`.xlsx`/`.csv` con esas columnas se reconoce sin escribir nada específico para ese banco — probado con ING e Ibercaja. El PDF, en cambio, no tiene una tabla real (solo texto posicionado) y cada banco lo maqueta distinto, así que ahí sí hace falta un parser dedicado por banco (Bankinter, BBVA).
- **Si el extracto trae "Concepto" y "Descripción" a la vez** (p. ej. Ibercaja: `Concepto` = tipo genérico de movimiento como "RECIBO", `Descripción` = comercio real), `findColumnMap` prioriza explícitamente `DESCRIP` sobre `CONCEPTO` — agrupar por el comercio concreto es lo útil aquí, no por el tipo de movimiento.

- **PDF sin librería de "extracción de tablas"**: ni Bankinter ni BBVA tienen una tabla real en su PDF, solo texto posicionado. `data/readPdfStatement.ts` reconstruye cada línea agrupando los fragmentos de texto de `pdfjs-dist` por su coordenada Y (misma fila) y ordenándolos por X (orden de columnas); `parseBankinterPdfLines`/`parseBbvaPdfLines` extraen los movimientos con expresiones regulares sobre esas líneas ya reconstruidas.
- **Dos layouts de PDF completamente distintos, dos estrategias distintas de importe.** Bankinter no imprime el importe de cada movimiento, solo el saldo resultante: el texto reconstruido pierde además la alineación que distinguiría cargo de abono en una columna fija, así que `parseBankinterPdfLines` arrastra el "saldo anterior" de cada sección y calcula cada importe como `saldo_nuevo − saldo_anterior` — el signo sale solo, y de paso sirve de comprobación (la suma tiene que cuadrar con el saldo final impreso). BBVA sí imprime el importe con signo en la propia línea del movimiento, así que `parseBbvaPdfLines` lo lee directamente sin necesidad de arrastrar ningún saldo.
- **BBVA no imprime el año en cada movimiento** (solo `DD/MM`): se toma del encabezado "Fecha de emisión" del extracto, asumiendo el año anterior si el mes del movimiento es posterior al de emisión (caso de un extracto de enero con algún movimiento de finales de diciembre).
- **No hay forma fiable de saber a qué banco pertenece un PDF sin intentar parsearlo** (la extensión es la misma para los dos): `readStatementFile.ts` prueba primero `parseBankinterPdfLines` y, si no reconoce ninguna sección, cae a `parseBbvaPdfLines`. Añadir un tercer banco en PDF es añadir su propio `parseXPdfLines` a esta misma cadena.
...
- **Guardar un snapshot dispara también un backup real de la base de datos**, no solo el diario por `launchd`: `POST /backup` (`backend/src/infrastructure/backup.ts`) ejecuta el mismo `scripts/backup-db.sh` que ya usaba el cron, en vez de reimplementar el `pg_dump` por segundo sitio. `runBackup` se inyecta como parámetro en `registerBackupRoutes` (no se importa dentro), precisamente para poder testear el contrato HTTP (200 con los datos del dump / 500 con un mensaje claro) sin depender de Docker en la suite automática — el backup real solo se verifica a mano. Un fallo del backup (Docker parado, etc.) nunca deshace ni bloquea el snapshot, que ya se ha guardado de verdad: el frontend distingue "snapshot guardado" de "backup hecho" y avisa de cada uno por separado.

## Login de un solo usuario (sin tabla `users`)

La API no exige login por defecto: el uso local/Tailscale sigue sin fricción. Pero desde que la app se desplegó en un servidor propio y accesible desde internet (ver "Despliegue en producción" más abajo), la instancia pública sí lo exige — son datos financieros reales, más sensibles todavía que cualquier otro proyecto personal desplegado hasta ahora.

`infrastructure/http/auth.ts` implementa el login: usuario/contraseña -> token de sesión firmado, sin tabla `users` ni estado en el servidor. Activado por tres variables de entorno juntas o ninguna (`LOGIN_USERNAME`/`LOGIN_PASSWORD_HASH`/`SESSION_SECRET`, ver `backend/.env.example`):

- **Hash de contraseña con scrypt** (nativo de Node, sin añadir bcrypt/argon2 solo para esto) — `infrastructure/http/passwordHash.ts`, formato `"salt:hash"` en hex. `npm run hash-password -- "contraseña"` genera el valor para `LOGIN_PASSWORD_HASH`; la contraseña en claro no se guarda en ningún sitio.
- **Token de sesión HMAC-SHA256** (`infrastructure/http/sessionToken.ts`), 30 días de validez, sin lista de revocación en el servidor — cerrar sesión solo borra el token guardado en el propio navegador.
- `GET /auth-config` siempre está disponible (incluso sin login configurado) y devuelve `{ loginRequired: boolean }`, para que el frontend sepa si debe mostrar el login antes de intentar nada, sin necesitar credenciales para preguntarlo.
- El frontend (`App.tsx`) no dispara ninguna petición de datos hasta saber si hace falta login o hay sesión ya guardada (`dataEnabled` pasado a `useFinancialData`) — evita una ráfaga de 401 en cada carga.
...
- **Base de datos propia, no gestionada por un tercero.** En local, Postgres en Docker; en producción, Postgres local en el propio VPS (ver "Despliegue en producción" más abajo) — nunca un proveedor de base de datos gestionada. Se evaluó explícitamente Supabase (había un servidor MCP ya conectado) frente a esta opción para el desarrollo local, y se optó por mantener los datos financieros reales fuera de cualquier proveedor externo.
...
- **Acceso local sin salir a internet.** El acceso remoto en el día a día se sigue haciendo por Tailscale (red privada punto a punto) cuando no hace falta el despliegue público — el backend escucha en todas las interfaces y el frontend llama a la API usando el mismo host desde el que se cargó la página (`window.location`), en vez de una URL fija (salvo que `VITE_API_URL` la fije, como en el build de producción), para que funcione igual en `localhost`, en red local o en Tailscale sin configuración extra.
...
8. **Zoom automático al enfocar un input en móvil**: iOS Safari (y algunos Android) hacen zoom al enfocar cualquier campo con `font-size` computado por debajo de 16px, dejando la vista desplazada al desenfocar — reportado en el input de nombre del modal de Perfil, pero el `text-sm` (14px) de `inputClass` es compartido por prácticamente todos los formularios de la app, así que el bug existía en todos ellos por igual. Corregido con una regla global (no una clase) — `input, select, textarea { font-size: 16px }` bajo `@media (max-width: 640px)` en `index.css` — para cubrir también cualquier campo que no use `inputClass` y cualquiera que se añada más adelante. Al estar fuera de cualquier `@layer`, esta regla gana sobre la utilidad `text-sm` de Tailwind (que sí vive dentro de `@layer utilities`) pase lo que pase con la especificidad de selector — verificado con el font-size computado real del input en un viewport de 390px de ancho.
...
- **Multi-banco limitado a tres formatos.** La importación de extractos solo reconoce ING (`.xls`/`.xlsx`/`.csv`), Bankinter y BBVA (PDF); añadir un banco nuevo significa escribir un parser nuevo (el patrón ya está puesto, pero no es automático).
