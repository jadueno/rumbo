...
- **`profile` es una tabla de exactamente una fila**, no un CRUD más: `GET/PUT /profile` sin id en la URL (`profileRoutes.ts`), y el repositorio hace `update` sin `where` porque solo puede haber una fila. Sustituye a lo que antes era `src/data/finances.ts` (config estática ignorada por git: edad fija, `accountFlows`, objetivo del fondo de emergencia) — la edad ahora se calcula en el dominio (`calculateAge`) a partir de una fecha de nacimiento real editable desde la pantalla "Perfil", en vez de ser un número que había que mantener a mano.
